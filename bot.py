...
# -------------------------------
# 🧮 SHOP + INLINE CALCULATOR
# -------------------------------
@bot.tree.command(name="shop", description="View items and calculate totals inline")
async def shop(interaction: discord.Interaction):
    ...
    for i in range(0, len(item_list), per_page):
        ...
        embed = discord.Embed(
            ...
            description="Browse and calculate totals directly below.\nUse the dropdown to select items and enter quantities.",
    ...
    class ShopCalculatorView(discord.ui.View):
        def __init__(self):
            super().__init__(timeout=180)
            self.page = 0
            self.selected_items = []
            self.total_embed = None
        ...
        def update_dropdown(self):
            ...
            self.select_menu = discord.ui.Select(
                placeholder="Select up to 5 items to add to calculator...",
                min_values=1,
                max_values=min(5, len(all_options)),
                options=all_options
            )
            self.select_menu.callback = self.handle_select
            self.add_item(self.select_menu)
            # Calculate + Clear
            self.add_item(self.calculate_button)
        ...
        async def update_message(self, interaction):
            embeds = [pages[self.page]]
            if self.total_embed:
                embeds.append(self.total_embed)
        ...
        async def calculate_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            class QuantityModal(discord.ui.Modal, title="Enter Quantities"):
                ...
                async def on_submit(self, interaction: discord.Interaction):
                    ...
                    total_embed = discord.Embed(title="📦 Total Calculation", color=discord.Color.blurple())
                    total_embed.add_field(name="💰 Total Buy", value=f"${total_buy:,.2f}")
                    total_embed.add_field(name="💵 Total Sell", value=f"${total_sell:,.2f}")
                    if details:
                        total_embed.add_field(name="🧾 Details", value="\n".join(details), inline=False)
                    if missing:
                        total_embed.add_field(name="⚠️ Invalid", value=", ".join(missing), inline=False)

                    self_view = self  # Capture outer scope
                    self_view.total_embed = total_embed
                    await interaction.response.edit_message(embeds=[pages[self_view.page], total_embed], view=self_view)

            await interaction.response.send_modal(QuantityModal(unique_selected))

        @discord.ui.button(label="🔁 Clear Total", style=discord.ButtonStyle.danger)
